...
下载模型可以通过`ollama run [model-name]`或者`ollama pull [model-name]`的方式。
...
执行`ollama show [model-name] --modelfile`来查看现有的modelfile, 例如：如果你准备复制`qwen2.5:0.5b`这个模型，你需要执行：
...
FROM /Users/sunguannan/.ollama/models/blobs/sha256-c5396e06af294bd101b30dce59131a76d2b773e76950acc870eda801d3ab0515
...
**注意看FROM这一行，要替换为你复制之后的新的路径。**

替换之后，将上面 modelfile 的内容复制到一个新的 modelfile 中。在 FROM 行中将地址替换为现有模型的 blob 地址。新文件名可以自定义，建议命名为 Modelfile。
